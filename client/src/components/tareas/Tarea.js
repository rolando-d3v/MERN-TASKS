...
export default function Tarea({ tarea }) {
  return (
    <li className="bg-white flex items-center my-2 py-2 justify-between px-6  space-x-2 rounded">
      ...
      <div>
        {tarea.estado ? (
          <button className="bg-green-200 px-2 py-1 rounded text-sm" type="button">
            Completo
          </button>
        ) : (
          <button className="bg-red-200 px-2 py-1 rounded text-sm" type="button">
            Incompleto
          </button>
        )}
      </div>
      <div className='flex-col space-y-2 justify-center w-1/4 ' >
